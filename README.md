...
如果发现光标形状不跟着变，比如鼠标移到文本框上还是那只普通猫，点菜单里的Calibrate for This Mac，它会接管你鼠标20秒左右学习你系统的光标长啥样，学完就正常了。这个只用做一次，以后系统大更新之后再点一次就行。

顺便说下为什么要这步：苹果每个大版本都会重画系统光标，我预置的数据是在macOS 26上采的，别的版本（比如Sonoma）对不上就认不出来就会出错

不想用了菜单里Quit一下，指针立马变回来。万一崩了系统也会自己把指针放出来，不会卡在没鼠标动不了的情况（
